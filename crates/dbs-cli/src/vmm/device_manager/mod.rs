...
use crate::vmm::address_space_manager::GuestAddressSpaceImpl;
use crate::utils::error::StartMicroVmError;
use crate::vmm::resource_manager::ResourceManager;
use crate::vmm::vm::{KernelConfigInfo, Vm};
...
pub enum DeviceMgrError {
    ...
    ResourceError(#[source] crate::vmm::resource_manager::ResourceError),
}
...
impl DeviceManager {
    ...
    pub fn create_mmio_virtio_device_with_features(
        ...
        let virtio_dev = match MmioV2Device::new(
            ctx.vm_fd.clone(),
            ctx.get_vm_as()?,
            ctx.irq_manager.clone() as Arc<dbs_interrup::KvmIrqManager>,
            device,
            resources,
            features,
        ) {
