...
// Reserve the 64MB MMIO address range just below 4G, x86 systems have special devices,
// such as LAPIC, IOAPIC, HPET etc, in this range. And we don't explicitly allocate MMIO
// address for those devices.
                    ...
                    match self.allocate_pio_address(&constraint) {
                        ...
                        None => {
                            if let Err(e) = self.free_device_resources(&resources) {
                                ...
                                return Err(ResourceError::NoAvailableResource);
                            }
                        }
                    }
                }
...
mod tests {
    ...
    #[test]
    fn test_allocate_legacy_irq() -> Result<(), ResourceError>{
    ...
    #[test]
    fn test_allocate_msi_irq() -> Result<(), ResourceError> {
        let mgr = ResourceManager::new(None);
    ...
    #[test]
    fn test_allocate_pio_addr() -> Result<(), ResourceError>{
        let mgr = ResourceManager::new(None);
